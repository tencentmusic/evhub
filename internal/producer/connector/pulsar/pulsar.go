...
func (s *Pulsar) SendDelayMsg(ctx context.Context, topic string, data []byte, delay time.Duration) error {
...
func (s *Pulsar) SendMsg(ctx context.Context, topic string, data []byte) error {
	p, err := s.getProducer(topic)
	if err != nil {
		return err
	}
